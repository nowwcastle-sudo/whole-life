...
def collect_claude():
    ...
    return version_out, [
        ...
        "- bare mode default: `-p` does not imply bare mode on this version; bare mode "
        "is reachable only via `--bare` or `CLAUDE_CODE_SIMPLE` (spec section 5). "
        "The gate that enforces this arrives with #13.",
    ]
...
def collect_codex():
    ...
    return version_out, [
        ...
        "ignores `config.toml` only and authentication is still read from `CODEX_HOME`",
    ]
...
def main():
    ...
    version, lines = (collect_claude if provider == "claude" else collect_codex)()
