...
- **tree 종료 계약이 보고하는 결말만이 아니라 raise 되는 결말도 이름한다** (#48).
  `terminate_process_tree`는 두 결말 — 확인된 reap, 그리고 발행됐지만 미확인, 둘 다
  반환값 — 을 완결된 집합처럼 생긴 문장으로 문서화하고 있었다. 셋째 결말은 제어 흐름이
  다르다: killer를 찾을 수 없으면 kill이 하나도 발행되기 전에 `LifecycleFailure`가
  raise 되는데, #34가 고친 두 호출처가 이것을 방어하는 것은 그 실패를 한 번 어렵게
  찾아냈기 때문일 뿐이었다. 동작은 의도적으로 그대로다 — 아무것도 시도하기 전의 raise는
  시끄럽게 실패하는 결말이고, 이를 반환값으로 옮기면 가드 없는 호출자가 조용히 정답이
  된다. 대신 계약이 이제 세 결말을 모두 이름하고, 계약이 raise를 이름하기를 멈추면
  테스트 하나가, 보고되던 결말이 raise로 바뀌면 다른 하나가 적색이 된다. 가드를 뺀
  호출자는 이제 계약을 따른 것이 아니라 무시한 것이 된다.
