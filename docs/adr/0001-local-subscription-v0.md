...
- **왜**: 여러 관점과 병렬 실행력을 얻으려면 1급 협업 agent 수와 각 provider harness 내부 worker 수를 분리해야 한다. Claude subagent는 parent에게만 결과를 돌려주고, Codex는 공식 subagent workflow의 parent sandbox를 상속한다. 「Claude가 재귀 spawn을 지원하지 않는다」고 적었던 부분은 2026-08-23 실측으로 정정했다(아래 정정 항목) — 지원하지 않는 것이 아니라 막지 않는다. 최소 공통 의미를 depth 1·read-only로 맞추면 broker가 보장할 수 없는 peer messaging이나 내부 context 공유를 약속하지 않으면서도 각 participant의 자율 분해 능력을 쓸 수 있다. Claude stable CLI에는 Codex와 같은 숫자 hard cap이 공식 확인되지 않았으므로 둘을 같은 보장으로 기록하지 않는다.
- **정정(2026-08-23)**: 위의 Claude depth는 처음에 `hard`로 기록했는데 실측이 다르게 나왔다. Claude Code 2.1.240에서 worker에게 자기 Agent 도구로 worker를 하나 더 띄우게 한 turn이 `spawn_depth=2`로 실행되어 성공으로 끝났고 `subagent_stats.refused.depth_limit`이 0이었다. 바뀐 것은 결정이 아니라 **provider가 그 결정을 대신 강제해 준다는 사실 기록**이다. depth 1·read-only라는 설계는 그대로이고, 이제 그 상한은 broker가 `spawn_depth` 관찰로 지킨다. 이것은 아래 뒤집는 조건에 해당하지 않는다 — 「막지 않는다」와 「동등하게 공식 지원한다」는 다른 사실이다. 같은 날 Codex 쪽도 정리했다 — 위 「정한 것」의 Codex 수치는 문서에서 온 기대치이고 위임 측정은 아직 실행되지 않았으므로, runtime은 Codex 세 축을 모두 `unsupported`로 보고하고 fail-closed한다. inline config의 동시성 상한도 `hard`로 기록하지 않는다 — 강제되는 것을 아무도 본 적이 없으면 그것은 측정이 아니라 주장이다.
